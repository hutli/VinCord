...
namespace VinCord
{
  public class HomeCommands : InteractionModuleBase<SocketInteractionContext>
  {
    ...
    public async Task GetHome()
    {
      ...
      await RespondAsync($"🏠 Home base: **{home}**");
    }

    [SlashCommand("sethome", "Sets the home base location (use absolute coordinates from CTRL+F3)")]
    [RequireUserPermission(GuildPermission.Administrator)]
    public async Task SetHome(
        [Summary("x", "absolute X coordinate (see CTRL+F3)")] int x,
        [Summary("y", "absolute Y coordinate")] int y,
        [Summary("z", "absolute Z coordinate (see CTRL+F3)")] int z)
    {
      // Convert pretty coordinates to absolute for internal storage
      var absolutePos = new BlockPos(x, y, z);
      _vincord.Config.HomeLocation = absolutePos;
      _vincord.SaveConfig();

      _vincord.Api.Server.LogNotification($"[VinCord] Home location set to absolute ({absolutePos})");
    ...
    public async Task Weather()
    {
      ...
      var embed = new EmbedBuilder()
          ...
          .AddField("📍 Location", _vincord.AbsoluteToPretty(home), inline: true)
